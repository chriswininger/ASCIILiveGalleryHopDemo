...
var colorCache = {};
var texts = new Texts(1000, 50, 0, function (entry) {
	entry.startY += 1;
	if (entry.startY > 50) entry.startY = 0;
});

var textsOverFlow = new Texts(1000, 50, -55, function (entry) {
    entry.startY += 1;
    if (entry.startY > 50) entry.startY = -55;
});
// global values to reduce overhead of var declaration in loop
var c, h, w, loopX, loopY, currentDist = null, rgbTest = null;
var Ascii = {
  ...
  toText: function(type, g, color, x, y, noVid) {
    var self = this;
    if (noVid) return color + (texts.getAnyText(x,y)  ||  ' ') + colors.console.default;
  	if (g <= 30) {
		c = '#';
    } else if (g > 30 && g <= 60) {
		c = '&';
